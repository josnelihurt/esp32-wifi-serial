...
template <size_t N>
using fixed_string = etl::string<N>;
...
inline etl::string<N> make_fixed_string(const span<const uint8_t>& data) noexcept {
...
inline small_string make_small_string(const span<const uint8_t>& data) noexcept {
  return make_fixed_string<64>(data);
}

inline large_string make_large_string(const span<const uint8_t>& data) noexcept {
